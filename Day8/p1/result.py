    ...
    for elt in positions.keys():
        ...
        for idx, pos in enumerate(values):
            for idx2 in range(idx + 1, len(values)):  # Commence à idx + 1
                pos2 = values[idx2]
                new1 = (2 * pos[0] - pos2[0], 2 * pos[1] - pos2[1])
                new2 = (2 * pos2[0] - pos[0], 2 * pos2[1] - pos[1])
                
                print(pos, pos2)
                
                if 0 <= new1[0] < num_rows and 0 <= new1[1] < num_cols:
                    if new1 not in visited:
                        counter += 1
                        visited.add(new1)
                    print(pos, pos2, new1)
                
                if 0 <= new2[0] < num_rows and 0 <= new2[1] < num_cols:
